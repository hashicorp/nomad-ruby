...
module Nomad
  describe Client do
    ...
    context "#with_retries" do
      ...
      subject { described_class.new(address: "http://nomad.test") }

      Nomad::Client::RESCUED_EXCEPTIONS.each do |e|
        it "retries on #{e}" do
          stub_request(:get, "http://nomad.test/")
            .to_raise(e).then
            .to_return(status: 200, body: "{}")

          subject.with_retries(Nomad::HTTPConnectionError, options) do
            subject.get("/")
          end
        end
        ...
        it "raises after maximum attempts on #{e}" do
          ...
          }.to raise_error(Nomad::HTTPConnectionError)
        end
      ...
      (400..422).each do |code|
        it "does not retry on a #{code} response code" do
          ...
          expect {
            subject.get("/")
          }.to raise_error(Nomad::HTTPClientError)
        end
      end

      # (500..520).each do |code|
      (500..500).each do |code|
        it "retries on a #{code} response code" do
          stub_request(:get, "http://nomad.test/")
            .to_return(status: code).then
            .to_return(status: 200, body: "{}")

          subject.with_retries(Nomad::HTTPError, options) do
            subject.get("/")
          end
        end
        ...
        it "raises after maximum attempts on #{code}" do
          ...
          expect {
            subject.with_retries(Nomad::HTTPServerError, options) do
              subject.get("/")
            end
          }.to raise_error(Nomad::HTTPServerError)
        end
